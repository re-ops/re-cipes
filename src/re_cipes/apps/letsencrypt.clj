...
(def-inline {:depends #'re-cipes.apps.letsencrypt/dehydrated} dns-lexicon
  "lexicon setup"
  []
  (letfn [(pip []
            (script
             ("sudo" "/usr/bin/pip3" "install" "requests[security]")
             ("sudo" "/usr/bin/pip3" "install" "dns-lexicon")))]
    (let [hook "dehydrated.default.sh"
